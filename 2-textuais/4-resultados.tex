...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[
        ...
        actor/.style={
            ...
            fill=cSlate, text=white,
            font=\small\bfseries,
            minimum width=2.8cm, minimum height=0.85cm,
            align=center, inner sep=5pt,
            blur shadow={shadow blur steps=6, shadow xshift=0pt, shadow yshift=-1.5pt, shadow blur radius=3pt, shadow opacity=18}
        },
        card/.style={
            rectangle, rounded corners=4pt,
            draw=gray!30, fill=white,
            font=\scriptsize, align=center,
            minimum width=2.1cm, minimum height=1.05cm,
            inner sep=3pt, line width=0.5pt,
            blur shadow={shadow blur steps=4, shadow xshift=0.5pt, shadow yshift=-1pt, shadow blur radius=2pt, shadow opacity=12}
        },
        sidebar/.style={
            rectangle, rounded corners=0pt,
            minimum width=0.45cm, inner sep=0pt,
            rounded corners=3pt
        },
        layerlbl/.style={
            font=\sffamily\bfseries\tiny,
            rotate=90, anchor=south, text=white,
            inner sep=2pt
        },
        conn/.style={->, >=stealth, line width=0.9pt, cSlate!50},
        biconn/.style={<->, >=stealth, line width=0.9pt, cSlate!50},
        flowlbl/.style={font=\tiny\sffamily, fill=white, inner sep=1.5pt, text=cSlate!80, rounded corners=1pt}
    ]

    % ═══════════════════════════════════════
    %  ATORES (topo) — badges escuros
    % ═══════════════════════════════════════
    \node[actor] (trab) at (-4.5, 8) {\faUser\ Trabalhador};
    \node[actor] (emp)  at (0, 8)    {\faBuilding\ Empregador};
    \node[actor] (gov)  at (4.5, 8)  {\faUniversity\ Governo};

    % ═══════════════════════════════════════
    %  CAMADA 1 — INTERFACE  (azul)
    % ═══════════════════════════════════════
    \fill[cBlue!5, rounded corners=8pt] (-6.6, 5.35) rectangle (6.6, 7.15);
    \draw[cBlue!25, rounded corners=8pt, line width=0.5pt] (-6.6, 5.35) rectangle (6.6, 7.15);
    % Sidebar
    \fill[cBlue, rounded corners=3pt] (-6.6, 5.35) rectangle (-6.15, 7.15);
    \node[layerlbl] at (-6.375, 6.25) {INTERFACE};
    % Cards
    \node[card, draw=cBlue!30] (app)    at (-3.5, 6.25) {\faDesktop\\[2pt] App/Web\\[-1pt]do Trabalhador};
    \node[card, draw=cBlue!30] (folha)  at (0, 6.25)    {\faFileCode\\[2pt] Sistema\\[-1pt]de Folha};
    \node[card, draw=cBlue!30] (painel) at (3.5, 6.25)  {\faUniversity\\[2pt] Painel\\[-1pt]Governamental};

    % Setas atores → interface
    \draw[conn] (trab) -- (app);
    \draw[conn] (emp) -- (folha);
    \draw[conn] (gov) -- (painel);

    % ═══════════════════════════════════════
    %  CAMADA 2 — GOVERNANÇA  (âmbar)
    % ═══════════════════════════════════════
    \fill[cAmber!5, rounded corners=8pt] (-6.6, 3.15) rectangle (6.6, 5.0);
    \draw[cAmber!25, rounded corners=8pt, line width=0.5pt] (-6.6, 3.15) rectangle (6.6, 5.0);
    % Sidebar
    \fill[cAmber, rounded corners=3pt] (-6.6, 3.15) rectangle (-6.15, 5.0);
    \node[layerlbl] at (-6.375, 4.075) {GOVERNANÇA};
    % Cards
    \node[card, draw=cAmber!40] (k1) at (-3.5, 4.1) {\faKey\\[2pt] Chave\\[-1pt]Trabalhador};
    \node[card, draw=cAmber!40] (k2) at (0, 4.1)    {\faKey\\[2pt] Chave\\[-1pt]Governo};
    \node[card, draw=cAmber!40] (k3) at (3.5, 4.1)  {\faKey\\[2pt] Chave\\[-1pt]Auditoria};
    % Badge multi-sig
    \node[fill=cAmber!12, draw=cAmber!40, rounded corners=3pt, font=\sffamily\tiny\bfseries, text=cAmber!80!black, inner sep=3pt, line width=0.4pt] at (5.5, 4.1) {Multi-sig 2-de-3};

    % Setas interface → governança
    \draw[conn] (app) -- (k1);
    \draw[conn] (folha) -- (k2) node[flowlbl, midway, right, xshift=1pt] { };
    \draw[conn] (painel) -- (k3);

    % ═══════════════════════════════════════
    %  CAMADA 3 — SMART CONTRACTS  (verde)
    % ═══════════════════════════════════════
    \fill[cGreen!5, rounded corners=8pt] (-6.6, 0.85) rectangle (6.6, 2.8);
    \draw[cGreen!25, rounded corners=8pt, line width=0.5pt] (-6.6, 0.85) rectangle (6.6, 2.8);
    % Sidebar
    \fill[cGreen, rounded corners=3pt] (-6.6, 0.85) rectangle (-6.15, 2.8);
    \node[layerlbl] at (-6.375, 1.825) {CONTRATOS};
    % Cards
    \node[card, draw=cGreen!35] (c1) at (-4.2, 1.85) {\faPiggyBank\\[2pt] Contribuição};
    \node[card, draw=cGreen!35] (c2) at (-1.4, 1.85) {\faChartLine\\[2pt] Investimento};
    \node[card, draw=cGreen!35] (c3) at (1.4, 1.85)  {\faUsers\\[2pt] Herança};
    \node[card, draw=cGreen!35] (c4) at (4.2, 1.85)  {\faUserShield\\[2pt] Compliance};

    % Setas governança → contratos
    \draw[conn] (k1.south) -- ++(0,-0.3) -| (c1.north);
    \draw[conn] (k2.south) -- ++(0,-0.15) -| (c2.north);
    \draw[conn] (k2.south) -- ++(0,-0.15) -| (c3.north);
    \draw[conn] (k3.south) -- ++(0,-0.3) -| (c4.north);

    % ═══════════════════════════════════════
    %  CAMADA 4 — BLOCKCHAIN  (violeta)
    % ═══════════════════════════════════════
    \fill[cViolet!5, rounded corners=8pt] (-6.6, -1.45) rectangle (6.6, 0.5);
    \draw[cViolet!25, rounded corners=8pt, line width=0.5pt] (-6.6, -1.45) rectangle (6.6, 0.5);
    % Sidebar
    \fill[cViolet, rounded corners=3pt] (-6.6, -1.45) rectangle (-6.15, 0.5);
    \node[layerlbl] at (-6.375, -0.475) {BLOCKCHAIN};
    % Cards
    \node[card, draw=cViolet!35] (t1) at (-3.5, -0.45) {\faCoins\\[2pt] Tokens\\[-1pt]BRL / Ativos};
    \node[card, draw=cViolet!35] (t2) at (0, -0.45)    {\faBook\\[2pt] Livro de\\[-1pt]Ordens};
    \node[card, draw=cViolet!35] (t3) at (3.5, -0.45)  {\faLock\\[2pt] Módulo de\\[-1pt]Privacidade};

    % Setas contratos → blockchain
    \draw[conn] (c1.south) -- ++(0,-0.2) -| (t1.north);
    \draw[conn] (c2.south) -- (t2.north);
    \draw[conn] (c3.south) -- ++(0,-0.1) -| (t2.north east);
    \draw[conn] (c4.south) -- ++(0,-0.2) -| (t3.north);

    % ═══════════════════════════════════════
    %  ENTIDADES EXTERNAS (base)
    % ═══════════════════════════════════════
    \node[actor] (b3)    at (-3.5, -2.8) {\faChartBar\ B3};
    \node[actor] (bacen) at (0, -2.8)    {\faLandmark\ Bacen};
    \node[actor] (nos)   at (3.5, -2.8)  {\faServer\ Validadores};

    % Setas blockchain ↔ entidades
    \draw[biconn] (t1) -- (b3);
    \draw[biconn] (t2) -- (bacen);
    \draw[biconn] (t3) -- (nos);
...
\begin{alineas}
    \item \textbf{Executar Trades}: Pode comprar e vender ativos livremente dentro do universo de investimentos permitido, respeitando o limite de 1 operação por mês. A operação requer co-assinatura automática do governo;
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[
        ...
        darrow/.style={->, >=stealth, thick, dashed, shorten >=1pt, shorten <=1pt}
    ]
        % ============================================================
        %  LINHA SUPERIOR — Valores reais (visíveis só aos participantes)
        % ============================================================
        \node[font=\small\bfseries] at (0, 6.5) {Visão do Remetente (valores reais)};

        % Caixa de fundo — valores reais
        \fill[gray!6, rounded corners=8pt] (-5.8, 3.2) rectangle (5.8, 5.4);
        \draw[gray!40, rounded corners=8pt] (-5.8, 3.2) rectangle (5.8, 5.4);

        % Entradas reais
        \node[rectangle, rounded corners=4pt, draw=black, fill=white, minimum width=2.2cm, minimum height=0.8cm, font=\small, align=center] (in1) at (-3.5, 4.8) {\faCoins\ \textbf{7 XMR}};
        \node[rectangle, rounded corners=4pt, draw=black, fill=white, minimum width=2.2cm, minimum height=0.8cm, font=\small, align=center] (in2) at (-3.5, 3.7) {\faCoins\ \textbf{5 XMR}};
        \node[font=\scriptsize, gray!60!black] at (-3.5, 5.8) {Entradas};

        % Sinal de igual
        \node[font=\Large] at (0, 4.25) {$\longrightarrow$};

        % Saídas reais
        \node[rectangle, rounded corners=4pt, draw=black, fill=white, minimum width=2.2cm, minimum height=0.8cm, font=\small, align=center] (out1) at (3.5, 4.8) {\faCoins\ \textbf{9 XMR}};
        \node[rectangle, rounded corners=4pt, draw=black, fill=white, minimum width=2.2cm, minimum height=0.8cm, font=\small, align=center] (out2) at (3.5, 3.7) {\faCoins\ \textbf{3 XMR}};
        \node[font=\scriptsize, gray!60!black] at (3.5, 5.8) {Saídas};

        % Soma
        \node[font=\scriptsize, align=center] at (-0.9, 4.8) {$7 + 5$};
        \node[font=\scriptsize, align=center] at (0.9, 4.8) {$9 + 3$};
        \node[font=\scriptsize\bfseries] at (0, 3.5) {$12 = 12$ \faCheck};

        % ============================================================
        %  SETAS DE TRANSFORMAÇÃO — Pedersen Commitment
        % ============================================================
        \draw[arrow, gray!70, line width=1.2pt] (-3.5, 3.15) -- (-3.5, 2.05) node[midway, left, font=\scriptsize, text=black] {$C = vG + rH$};
        \draw[arrow, gray!70, line width=1.2pt] (3.5, 3.15) -- (3.5, 2.05) node[midway, right, font=\scriptsize, text=black] {$C = vG + rH$};

        \node[font=\small\bfseries, fill=white, inner sep=3pt] at (0, 2.8) {\faLock\ Pedersen Commitment};

        % ============================================================
        %  LINHA INFERIOR — O que a blockchain registra (público)
        % ============================================================
        \node[font=\small\bfseries] at (0, 2) {Visão da Blockchain (valores ocultos)};

        % Caixa de fundo — commitments
        \fill[gray!18, rounded corners=8pt] (-5.8, -1.2) rectangle (5.8, 1.2);
        \draw[gray!50, rounded corners=8pt] (-5.8, -1.2) rectangle (5.8, 1.2);

        % Commitments de entrada
        \node[rectangle, rounded corners=4pt, draw=black, fill=gray!30, minimum width=2.6cm, minimum height=0.8cm, font=\small, align=center] (cin1) at (-3.5, 0.6) {$C_{in_1}$ \faQuestion};
        \node[rectangle, rounded corners=4pt, draw=black, fill=gray!30, minimum width=2.6cm, minimum height=0.8cm, font=\small, align=center] (cin2) at (-3.5, -0.5) {$C_{in_2}$ \faQuestion};
        \node[font=\scriptsize, gray!60!black] at (-3.5, 1.5) {Entradas ocultas};

        % Seta central
        \node[font=\Large] at (0, 0.05) {$\longrightarrow$};

        % Commitments de saída
        \node[rectangle, rounded corners=4pt, draw=black, fill=gray!30, minimum width=2.6cm, minimum height=0.8cm, font=\small, align=center] (cout1) at (3.5, 0.6) {$C_{out_1}$ \faQuestion};
        \node[rectangle, rounded corners=4pt, draw=black, fill=gray!30, minimum width=2.6cm, minimum height=0.8cm, font=\small, align=center] (cout2) at (3.5, -0.5) {$C_{out_2}$ \faQuestion};
        \node[font=\scriptsize, gray!60!black] at (3.5, 1.5) {Saídas ocultas};

        % ============================================================
        %  VERIFICAÇÃO
        % ============================================================
        \node[rectangle, rounded corners=6pt, draw=black, dashed, fill=white, minimum width=10cm, minimum height=0.9cm, align=center, font=\small] (proof) at (0, -2.2) {\faCheck\ \ Rede verifica: $\displaystyle\sum C_{in} - \sum C_{out} = 0$ \quad sem conhecer os valores};
        \draw[darrow] (0, -1.25) -- (proof.north);

        % ============================================================
        %  OBSERVADOR
        % ============================================================
        \node[rectangle, rounded corners=6pt, draw=black, fill=gray!40, minimum width=10cm, minimum height=0.9cm, align=center, font=\scriptsize] (obs) at (0, -3.6) {\faEyeSlash\quad \textbf{Observador externo:} vê $C_{in_1}, C_{in_2} \rightarrow C_{out_1}, C_{out_2}$ --- valores e identidades permanecem ocultos};
        \draw[darrow] (proof.south) -- (obs.north);
