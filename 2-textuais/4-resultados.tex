...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[
        ...
        actor/.style={rectangle, rounded corners=4pt, draw=black!70, fill=white, minimum width=2.2cm, minimum height=0.9cm, align=center, font=\scriptsize},
        contract/.style={rectangle, rounded corners=3pt, draw=black!60, fill=gray!12, minimum width=2.0cm, minimum height=0.7cm, align=center, font=\scriptsize},
        layer/.style={rectangle, rounded corners=6pt, draw=gray!50, minimum height=1.6cm, align=center, font=\scriptsize, line width=0.6pt},
        conn/.style={->, >=stealth, thick, gray!60},
        biconn/.style={<->, >=stealth, thick, gray!60},
        lbl/.style={font=\tiny, text=gray!70!black, fill=white, inner sep=1pt}
    ]
        % ====================================================================
        %  ATORES EXTERNOS (topo)
        % ====================================================================
        \node[actor, fill=gray!8] (trab) at (-4.5, 5.5) {\faUser\ \textbf{Trabalhador}};
        \node[actor, fill=gray!8] (emp)  at (0, 5.5)    {\faBuilding\ \textbf{Empregador}};
        \node[actor, fill=gray!8] (gov)  at (4.5, 5.5)  {\faUniversity\ \textbf{Governo}};

        % ====================================================================
        %  CAMADA 1 — Interface (App / Web)
        % ====================================================================
        \fill[gray!5, rounded corners=6pt] (-6.3, 3.6) rectangle (6.3, 4.9);
        \draw[gray!40, rounded corners=6pt, line width=0.6pt] (-6.3, 3.6) rectangle (6.3, 4.9);
        \node[font=\scriptsize\bfseries, gray!60!black] at (-4.8, 4.65) {\faDesktop\ Interface};

        \node[contract] (app) at (-2.5, 4.25) {App/Web\\do Trabalhador};
        \node[contract] (folha) at (1.0, 4.25) {Sistema\\de Folha};
        \node[contract] (painel) at (4.5, 4.25) {Painel\\Governamental};

        % Conexões atores → interface
        \draw[conn] (trab) -- (app);
        \draw[conn] (emp) -- (folha);
        \draw[conn] (gov) -- (painel);

        % ====================================================================
        %  CAMADA 2 — Governança Multi-sig 2-de-3
        % ====================================================================
        \fill[gray!10, rounded corners=6pt] (-6.3, 1.8) rectangle (6.3, 3.3);
        \draw[gray!40, rounded corners=6pt, line width=0.6pt] (-6.3, 1.8) rectangle (6.3, 3.3);
        \node[font=\scriptsize\bfseries, gray!60!black] at (-4.5, 3.05) {\faKey\ Governança};

        \node[contract] (keytrab) at (-3.5, 2.55) {\faKey\ Chave\\Trabalhador};
        \node[contract] (keygov)  at (0, 2.55)    {\faKey\ Chave\\Governo};
        \node[contract] (keyaud)  at (3.5, 2.55)  {\faKey\ Chave\\Auditoria};

        % Multi-sig label
        \node[font=\tiny\bfseries, text=black!60] at (0, 2.0) {Carteira Multi-sig 2-de-3};

        % Conexões interface → governança
        \draw[conn] (app) -- (keytrab);
        \draw[conn] (painel) -- (keygov);

        % ====================================================================
        %  CAMADA 3 — Smart Contracts
        % ====================================================================
        \fill[gray!15, rounded corners=6pt] (-6.3, -0.4) rectangle (6.3, 1.5);
        \draw[gray!40, rounded corners=6pt, line width=0.6pt] (-6.3, -0.4) rectangle (6.3, 1.5);
        \node[font=\scriptsize\bfseries, gray!60!black] at (-4.3, 1.25) {\faFileCode\ Smart Contracts};

        \node[contract] (contrib) at (-4.2, 0.55) {\faPiggyBank\\Contribuição};
        \node[contract] (invest) at (-1.4, 0.55)  {\faChartLine\\Investimento};
        \node[contract] (herd) at (1.4, 0.55)     {\faUsers\\Herança};
        \node[contract] (compl) at (4.2, 0.55)    {\faUserShield\\Compliance};

        % Conexão governança → contracts
        \draw[conn] (0, 1.8) -- (0, 1.5);

        % ====================================================================
        %  CAMADA 4 — Mercado Tokenizado + Blockchain
        % ====================================================================
        \fill[gray!20, rounded corners=6pt] (-6.3, -2.8) rectangle (6.3, -0.7);
        \draw[gray!40, rounded corners=6pt, line width=0.6pt] (-6.3, -2.8) rectangle (6.3, -0.7);
        \node[font=\scriptsize\bfseries, gray!60!black] at (-4.1, -0.95) {\faLink\ Blockchain Permissionada};

        \node[contract] (token) at (-3.5, -1.75) {\faCoins\ Tokens\\BRL / Ativos};
        \node[contract] (livro) at (0, -1.75)    {\faBook\ Livro de\\Ordens};
        \node[contract] (priv) at (3.5, -1.75)  {\faLock\ Módulo de\\Privacidade};

        % Conexão contracts → blockchain
        \draw[conn] (-1.4, -0.4) -- (-1.4, -0.7);
        \draw[conn] (1.4, -0.4) -- (1.4, -0.7);

        % ====================================================================
        %  MUNDO EXTERNO (base)
        % ====================================================================
        \node[actor, fill=gray!8] (b3)    at (-3.5, -3.8) {\faChartBar\ \textbf{B3}};
        \node[actor, fill=gray!8] (bacen) at (0, -3.8)    {\faLandmark\ \textbf{Bacen}};
        \node[actor, fill=gray!8] (noval) at (3.5, -3.8)  {\faServer\ \textbf{Nós Validadores}};

        % Conexões blockchain → mundo externo
        \draw[biconn] (token) -- (b3);
        \draw[biconn] (livro) -- (bacen);
        \draw[biconn] (priv) -- (noval);

        % ====================================================================
        %  LABELS nas setas de desconto em folha
        % ====================================================================
        \draw[conn, black!50] (folha) -- (contrib) node[lbl, midway, right, xshift=2pt] {R\$ $\rightarrow$ BRL};
