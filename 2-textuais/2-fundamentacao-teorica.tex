...
\subsection{O Teorema do Júri de Condorcet}

A superioridade da decisão coletiva sobre a individual possui fundamentação matemática rigorosa. O \textbf{Teorema do Júri de Condorcet}, proposto pelo Marquês de Condorcet em 1785, demonstra que, em uma votação binária (certo \textit{versus} errado), se cada votante possui probabilidade individual $p > 0{,}5$ de tomar a decisão correta, então a probabilidade de a maioria decidir corretamente converge para 1 à medida que o número de votantes $n$ aumenta \cite{condorcet1785}.

Formalmente, considere $n$ votantes independentes, cada um com probabilidade $p$ de acertar. A probabilidade de a maioria estar correta é dada por:

\vspace{0.2cm}
\noindent\textbf{Onde:}
\begin{itemize}
    \item $n$ é o número total de votantes;
    \item $k$ é o número de votantes que acertam (ou seja, que votam corretamente).
\end{itemize}
\vspace{-0.2cm}
...
\noindent onde $\binom{n}{k}$ é o coeficiente binomial. O teorema estabelece que:
...
\begin{equation}
    ...
    \begin{cases}
        ...
        0, & \text{se } p < 0{,}5
    \end{cases}
    \label{eq:condorcet-limite}
\end{equation}

    \vspace{0.2cm}
    \noindent\textbf{Exemplo numérico:}

    A soma é feita de $k = 3$ até $k = 5$:
    \begin{equation*}
        P_5 = \sum_{k=3}^{5} \binom{5}{k} p^k (1-p)^{5-k}
    \end{equation*}

    Seja $p = 0{,}6$ (cada um acerta com 60\% de chance):
    \begin{itemize}
        \item Para $k = 3$: $\binom{5}{3} \cdot 0{,}6^3 \cdot 0{,}4^2 = 10 \cdot 0{,}216 \cdot 0{,}16 = 0{,}3456$
        \item Para $k = 4$: $\binom{5}{4} \cdot 0{,}6^4 \cdot 0{,}4^1 = 5 \cdot 0{,}1296 \cdot 0{,}4 = 0{,}2592$
        \item Para $k = 5$: $\binom{5}{5} \cdot 0{,}6^5 \cdot 0{,}4^0 = 1 \cdot 0{,}07776 \cdot 1 = 0{,}07776$
    \end{itemize}
    ...
    Ou seja, com 5 votantes independentes e $p = 0{,}6$, a chance da maioria acertar é $\approx 68\%$.

A Figura \ref{fig:condorcet-convergencia} ilustra essa convergência para diferentes valores de $p$. Observa-se que, mesmo com competência individual modesta ($p = 0{,}6$), poucos dezenas de agentes já bastam para que a decisão coletiva se aproxime da certeza. Inversamente, quando $p < 0{,}5$, mais participantes \textit{pioram} o resultado.

\begin{figure}[htb]
    \centering
    \caption{Convergência da probabilidade de decisão correta pela maioria em função do número de votantes, para diferentes competências individuais $p$.}
    \label{fig:condorcet-convergencia}
    \includegraphics[width=0.85\textwidth]{figuras/condorcet_convergencia.pdf}
    \fonte{Elaborado pelo autor.}
\end{figure}

As premissas do teorema são: (i) votos independentes, (ii) competência individual $p > 0{,}5$, (iii) decisão binária e (iv) competência homogênea. Embora essas condições sejam idealizadas, o resultado captura a intuição fundamental de que \textbf{cada agente funciona como um sensor imperfeito}, e a agregação reduz o ruído individual.

Essa lógica se aplica diretamente à alocação de recursos financeiros. Quando milhões de trabalhadores direcionam suas poupanças de forma autônoma, o mecanismo de descoberta de preços agrega informações dispersas e tende a alocar capital de maneira mais eficiente do que a gestão centralizada por um único ente estatal --- fenômeno descrito por Hayek como o ``problema do conhecimento'' \cite{hayek1945}. É essa convergência estatística que fundamenta a proposta de permitir aos trabalhadores a gestão individual de seus recursos previdenciários.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[
        node distance=1.5cm,
        key/.style={circle, fill=gray!20, minimum size=1.4cm, font=\footnotesize, align=center},
        contract/.style={rectangle, fill=gray!35, rounded corners=8pt, minimum width=3.5cm, minimum height=1.5cm, font=\small, align=center},
        vault/.style={rectangle, fill=gray!50, rounded corners=3pt, minimum width=3cm, minimum height=1.2cm, font=\small, align=center},
        arrow/.style={->, thick, >=stealth, line width=1.2pt, shorten >=3pt, shorten <=3pt}
    ]
    ...
    % Labels das chaves
    \node[below=0.3cm of key1, font=\footnotesize, align=center, text width=2cm] (lbl1) {\textbf{Trabalhador} \\ {\scriptsize Chave 1}};
    \node[below=0.3cm of key2, font=\footnotesize, align=center, text width=2cm] (lbl2) {\textbf{Governo} \\ {\scriptsize Chave 2}};
    \node[below=0.3cm of key3, font=\footnotesize, align=center, text width=2cm] (lbl3) {\textbf{Auditoria} \\ {\scriptsize Chave 3}};
    
    % Smart Contract
    \node[contract] (smart) at (4, 0.5) {\faFileContract\ \textbf{Smart Contract 2-de-3}};
    
    % Carteira/Cofre
    \node[vault] (vault) at (4, -1.5) {\faLock\ \textbf{Carteira Previdenciária}};
    
    % Setas das chaves para o contrato
    \draw[arrow, color=black] (lbl1.south) -- (smart.north west);
    \draw[arrow, color=black] (lbl2.south) -- (smart.north);
    \draw[arrow, color=black] (lbl3.south) -- (smart.north east);
    
    % Seta do contrato para a carteira
    \draw[arrow, color=black, line width=2pt] (smart.south) -- node[right=3pt, font=\scriptsize, color=black] {Autorização} (vault.north);
    
    % Indicador de quórum
    \node[fill=gray!10, rounded corners=5pt, font=\scriptsize, text width=2.2cm, align=center] at (8.5, 1) {\faCheckDouble\ Mínimo: 2 assinaturas};
    
    % Cenários de uso
    \node[font=\scriptsize, align=left, text width=4.8cm, fill=gray!5, rounded corners=3pt, inner sep=6pt] at (-2.5, -0.5) {
        \textbf{Combinações válidas:} \\[3pt]
        {\color{black}\faCheck} Trabalhador + Governo {\tiny(rotineira)} \\[2pt]
        {\color{black}\faCheck} Governo + Auditoria {\tiny(recuperação)} \\[5pt]
        \textbf{Combinação inválida:} \\[3pt]
...
\subsection{O Que São Juros Reais}

Uma distinção fundamental --- e frequentemente ignorada pela população --- é a diferença entre \textbf{juros nominais} e \textbf{juros reais}. A taxa de juros nominal é o número que aparece no contrato ou no extrato bancário. A taxa de juros real, por outro lado, é o que efetivamente resta após descontar a inflação do período. A relação entre ambas é dada pela \textbf{Equação de Fisher}:
...
\noindent onde $i$ é a taxa de juros nominal, $r$ é a taxa de juros real e $\pi$ é a taxa de inflação. Rearranjando para isolar a taxa real:

\begin{equation}
\label{eq:fisher-real}
r = \frac{1 + i}{1 + \pi} - 1
\end{equation}

Quando os juros nominais são \textbf{inferiores} à inflação, os juros reais tornam-se \textbf{negativos} --- o que significa que o dinheiro aplicado \textbf{perde} poder de compra ao longo do tempo, mesmo aparentando ``render'' no extrato.

Para ilustrar o impacto concreto, considere um trabalhador que deposita R\$ 1.000{,}00 e mantém a aplicação por 20 anos. A Tabela \ref{tab:juros-reais} compara três cenários: juros reais positivos (investimento em ações com retorno médio de 8\% a.a. acima da inflação), juros reais zero (apenas correção monetária) e juros reais negativos (FGTS, que historicamente rendeu abaixo da inflação).

\begin{table}[htb]
    \centering
    \caption{Impacto dos Juros Reais sobre R\$ 1.000 em 20 anos (valores corrigidos pela inflação)}
    \label{tab:juros-reais}
    \begin{tabular}{lccc}
        \toprule
        \textbf{Cenário} & \textbf{Taxa real} & \textbf{Valor em 20 anos} & \textbf{Ganho/Perda real} \\
        \midrule
        Ações (bolsa brasileira) & +8\% a.a. & R\$ 4.660{,}96 & +366\% \\
        Tesouro IPCA+ & +5\% a.a. & R\$ 2.653{,}30 & +165\% \\
        Poupança & $\approx$0\% a.a. & $\approx$ R\$ 1.000{,}00 & 0\% \\
        FGTS (TR + 3\%) & $\approx$$-$2\% a.a. & R\$ 667{,}61 & $-$33\% \\
        \bottomrule
    \end{tabular}
    \fonte{LIMA (2026). Cálculos baseados em médias históricas.}
\end{table}

O resultado é revelador: enquanto R\$ 1.000 investidos em ações se transformam em mais de R\$ 4.600 em poder de compra real ao longo de 20 anos, os mesmos R\$ 1.000 depositados no FGTS se reduzem a aproximadamente R\$ 668 em termos reais. Ou seja, o trabalhador que confiou suas economias ao FGTS por duas décadas não apenas \textbf{não ganhou nada}: ele \textbf{perdeu} um terço do que depositou. Em 30 anos --- horizonte típico de uma vida laboral --- a perda real acumulada do FGTS pode chegar a 40\%, conforme estimado pelo Insper \cite{insper2019}.
...
A explicação é direta: a Terra gira sobre seu eixo com velocidade angular constante $\omega \approx 7{,}27 \times 10^{-5}$ rad/s. A velocidade tangencial $v$ de um ponto na superfície depende do raio efetivo naquela latitude $\phi$:

\begin{equation}
\label{eq:vel-tangencial}
v = \omega \cdot R_T \cdot \cos(\phi)
\end{equation}

\noindent onde $R_T \approx 6.371$ km é o raio médio da Terra. No Equador ($\phi = 0^{\circ}$), $\cos(0^{\circ}) = 1$ e a velocidade tangencial atinge seu valor máximo:

\begin{equation}
\label{eq:vel-equador}
v_{\text{eq}} = 7{,}27 \times 10^{-5} \times 6{,}371 \times 10^{6} \approx 463 \text{ m/s} \approx 1.667 \text{ km/h}
\end{equation}

\noindent Já em Cabo Canaveral ($\phi \approx 28{,}5^{\circ}$):

\begin{equation}
\label{eq:vel-canaveral}
v_{\text{CC}} = 463 \times \cos(28{,}5^{\circ}) \approx 463 \times 0{,}879 \approx 407 \text{ m/s}
\end{equation}

\noindent Em Sobral ($\phi \approx 3{,}7^{\circ}$):

\begin{equation}
\label{eq:vel-sobral}
v_{\text{Sobral}} = 463 \times \cos(3{,}7^{\circ}) \approx 463 \times 0{,}998 \approx 462 \text{ m/s}
\end{equation}

\noindent E em Alcântara ($\phi \approx 2{,}3^{\circ}$):

\begin{equation}
\label{eq:vel-alcantara}
v_{\text{Alc}} = 463 \times \cos(2{,}3^{\circ}) \approx 463 \times 0{,}999 \approx 462{,}6 \text{ m/s}
\end{equation}

A Tabela \ref{tab:velocidade-tangencial} resume os valores calculados. Nota-se que Sobral retém \textbf{99,8\%} da velocidade equatorial, perdendo apenas $\sim$1 m/s em relação ao Equador, enquanto Cabo Canaveral perde 56 m/s.

\begin{table}[htb]
    \centering
    \caption{Velocidade tangencial por localidade}
    \label{tab:velocidade-tangencial}
    \begin{tabular}{lccc}
        \toprule
        \textbf{Local} & \textbf{Latitude} & \textbf{$v$ (m/s)} & \textbf{$\Delta v$ vs Equador} \\
        \midrule
        Equador & 0° & 463,0 & 0 \\
        Alcântara (MA) & 2,3° S & 462,6 & $-$0,4 \\
        Sobral (CE) & 3,7° S & 462,0 & $-$1,0 \\
        Cabo Canaveral (EUA) & 28,5° N & 407,0 & $-$56,0 \\
        \bottomrule
    \end{tabular}
    \fonte{LIMA (2026), com base em \citeonline{wertz2011}.}
\end{table}

A diferença entre Sobral e Cabo Canaveral é de $\Delta v \approx 55$ m/s. Essa diferença pode parecer modesta, mas em astrodinâmica cada m/s adicional de $\Delta v$ exige combustível extra de forma \textbf{exponencial}, conforme a equação de Tsiolkovsky \cite{wertz2011}:
...
\noindent onde $v_e$ é a velocidade de exaustão dos gases, $m_0$ a massa inicial (com combustível) e $m_f$ a massa final (sem combustível). Rearranjando, a razão de massa necessária cresce exponencialmente com o $\Delta v$ requerido:

\begin{equation}
\label{eq:razao-massa}
\frac{m_0}{m_f} = e^{\Delta v / v_e}
\end{equation}

Para quantificar o ganho concreto de Sobral sobre Cabo Canaveral, considere um foguete típico com velocidade de exaustão $v_e \approx 3.000$ m/s (representativo de motores a querosene/LOX como o Merlin da SpaceX). A velocidade necessária para LEO é $\Delta v_{\text{LEO}} \approx 7.800$ m/s. Descontando o impulso gratuito da rotação terrestre:

\begin{equation}
\label{eq:dv-canaveral}
\Delta v_{\text{CC}} = 7.800 - 407 = 7.393 \text{ m/s}
\end{equation}

\begin{equation}
\label{eq:dv-sobral}
\Delta v_{\text{Sobral}} = 7.800 - 462 = 7.338 \text{ m/s}
\end{equation}

\noindent Aplicando a Equação \ref{eq:razao-massa}, a razão de massa necessária em cada local é:

\begin{equation}
\label{eq:razao-cc}
\left(\frac{m_0}{m_f}\right)_{\text{CC}} = e^{7393/3000} = e^{2{,}464} \approx 11{,}75
\end{equation}

\begin{equation}
\label{eq:razao-sobral}
\left(\frac{m_0}{m_f}\right)_{\text{Sobral}} = e^{7338/3000} = e^{2{,}446} \approx 11{,}54
\end{equation}

\noindent A diferença relativa na razão de massa é:

\begin{equation}
\label{eq:ganho-massa}
\text{Ganho} = 1 - \frac{11{,}54}{11{,}75} \approx 1{,}8\%
\end{equation}

Em termos práticos, para um foguete como o Falcon 9, com massa total de aproximadamente 549 toneladas \cite{spacex2023}, essa redução de 1,8\% na razão de massa representa cerca de \textbf{10 toneladas a menos de combustível} por lançamento --- ou, equivalentemente, 10 toneladas adicionais de carga útil. Considerando o custo médio de US\$ 2.720/kg para colocar carga em LEO \cite{spacex2023}, o ganho por lançamento é de aproximadamente \textbf{US\$ 27 milhões}, tornando Sobral uma alternativa economicamente superior a Cabo Canaveral.
