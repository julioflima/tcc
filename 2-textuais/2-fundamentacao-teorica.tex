...
Para utilizar o RSA, cada usuário gera seu próprio par de chaves seguindo estes passos \cite{tomaz2014, stallings2008}:

\begin{enumerate}
    \item \textbf{Escolher dois números primos grandes} $p$ e $q$, aleatórios e de tamanho semelhante (na prática, cada um com mais de 100 algarismos);
    \item \textbf{Calcular o módulo} $n = p \times q$. Este $n$ será parte de ambas as chaves;
    \item \textbf{Calcular a função totiente de Euler}: $\phi(n) = (p-1)(q-1)$. A função totiente indica quantos números inteiros entre $1$ e $n$ são primos relativos a $n$ --- ou seja, não compartilham nenhum fator com $n$ além do $1$. Este valor é essencial porque define o ``ciclo'' da aritmética modular do RSA;
    \item \textbf{Escolher o expoente público} $e$, um inteiro primo relativo a $\phi(n)$ (isto é, $\text{mdc}(e, \phi(n)) = 1$);
    \item \textbf{Calcular o expoente privado} $d$, tal que $e \cdot d \equiv 1 \pmod{\phi(n)}$ --- em outras palavras, $d$ é o inverso multiplicativo de $e$ módulo $\phi(n)$.
\end{enumerate}

Ao final, os números $p$ e $q$ devem ser descartados. As chaves resultantes são:

\begin{alineas}
    \item \textbf{Chave pública}: o par $(e, n)$;
    \item \textbf{Chave privada}: o par $(d, n)$.
\end{alineas}
